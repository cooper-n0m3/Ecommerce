...
import ImageStepper from "../../product/components/ImageStepper";

const FileUploader = ({ name, name2, draftImage = [], nameOldImage = "" ,statusSubmit=false}) => {
  const { setFieldValue } = useFormikContext();
  const [previewOpen, setPreviewOpen] = useState(false);
  const [previewImage, setPreviewImage] = useState("");
  const [preview, setPreview] = useState([]);
  const [deleteImage, setDeleteImage] = useState([]);
  const [getDraftImage, setGetDraftImage] = useState([]);
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length + getDraftImage?.length > 4) {
      return toast.error("Sorry images are available 4 slots!");
    }
    if (preview.length + getDraftImage?.length === 4) {
  ...
  const handleRemoveDraftImage = (filename) => {
    setGetDraftImage((prevFiles) =>prevFiles.filter((item) => item !== filename));
    setDeleteImage(getDraftImage.filter((item)=>item!=filename))
  };
  ...
  //delete image to update in dv-
  useEffect(() => {
    if (deleteImage?.length > 0) {
      // console.log(deleteImage);
      setFieldValue(nameOldImage, deleteImage);
    }else{
      setFieldValue(nameOldImage, '');
    }
  }, [deleteImage]);

  //draft image set state
  useEffect(() => {
    if (draftImage?.length > 0) {
      setGetDraftImage(draftImage);
      setDeleteImage(draftImage);
    }
  }, [draftImage]);
  ...
  const handleClickFile = (fieldName) => {
    ...
    if (preview.length + getDraftImage.length < 4) {
      ...
      toast.error("images are full now!");
    }
  };

  //get status submit form
  useEffect(()=>{
    if(statusSubmit){
      setPreview([])
      setDeleteImage([])
  ...
  return (
    <div className="grid grid-cols-5 gap-2 border-y p-2 border-opacity-10 border-gray-400">
      <div className="col-span-2   items-center">
        {getDraftImage.length == 0 && (
          <div
          ...
          >
            <div className="box-upload transform transition-transform duration-300 group-hover:scale-110 flex justify-center items-center w-[50px] rounded-full aspect-square bg-opacity-60 bg-gray-600">
              <svg
                className="w-7 shadow transform transition-transform duration-300 group-hover:scale-110 h-7 p-1 bg-gray-400 rounded-full bg-opacity-65 text-gray-800 dark:text-white"
                aria-hidden="true"
        ...
        {getDraftImage.length > 0 &&  (
          <>
            <div
              onClick={(e) => handleClickFile(name2)}
              className={`${
                preview.length+getDraftImage.length  >= 4
                  ? "cursor-no-drop"
                  : "cursor-pointer"
              } group  mb-1 w-full gap-5 min-h-[240px] flex flex-col justify-center items-center border border-dotted hover:border-dashed  border-gray-500 rounded bg-gray-700 bg-opacity-35`}
            >
              <div className="box-upload  transform transition-transform duration-300 group-hover:scale-110 flex justify-center items-center w-[50px] rounded-full aspect-square bg-opacity-60 bg-gray-600">
                <svg
                  className="w-7 shadow transform transition-transform duration-300 group-hover:scale-110 h-7 p-1 bg-gray-400 rounded-full bg-opacity-65 text-gray-800 dark:text-white"
              ...
              <p className="text-[13px] text-gray-400">
                <span className="text-blue-500">Browse</span> or drag files
                here(<font color="white">alt+u</font>)
              </p>
              <input
                className="border hidden border-gray-400 rounded bg-gray-700 text-gray-400 focus:text-gray-300"
                type="file"
                ...
                name={name2}
                id={name2}
                onChange={handleFileChange} // Reuse the handler for multiple inputs
              />
            </div>
            <ImageStepper
              name={name}
              overalStep={5}
              beginStep={getDraftImage?.length}
            />
          </>
        )}
        ...
        <input
          type="hidden"
          name={nameOldImage}
          defaultValue={deleteImage} // Controlled input
      ...
      <div className="col-span-3">
        <AnimatePresence transition={{ duration: 0.5 }}>
              ...
              {file.isValid && (
                <div className=" w-2/12  rounded h-full divide-x divide-gray-700  border-gray-600 border-opacity-50 p-[3px] flex items-center justify-between">
                  ...
                  >
                    ...
                    >
                      <svg
                        className="w-[15px] h-[15px] group-hover:scale-125 group-hover:text-blue-600 transform transition-transform duration-500 text-gray-800 dark:text-gray-400"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg"
                        width={24}
                        height={24}
                  ...
                  >
                    ...
                    >
                      <svg
                        className="w-[18px] h-[18px] group-hover:scale-125 group-hover:text-red-600 transform transition-transform duration-500 text-gray-800 dark:text-gray-400"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg"
                        width={24}
                        height={24}
          ...
          {preview.length > 0 && (
            <p className="text-gray-400 text-[14px] float-end">
              preview-selected
            </p>
          )}
        </AnimatePresence>
        <AnimatePresence transition={{ duration: 0.5 }}>
          {getDraftImage?.length > 0 && 
            getDraftImage.map((filename, index) => (
              <motionFrame.div
                initial={{ opacity: 1, transitionDuration: 0.5 }}
                animate={{ opacity: 1, transitionDuration: 0.5 }}
                exit={{ opacity: 0 }}
                style={{ listStyle: "none", marginBottom: "3px" }}
                key={index}
                className={`w-full  bg-gray-700 bg-opacity-40 p-1 flex items-center h-[58px] gap-3 border border-green-500 
                } border-opacity-20 rounded `}
              >
                <div className="w-1/12 border border-gray-400 border-opacity-70 rounded h-full flex items-center justify-start">
                  <div
                  ...
                  >
                    ...
                    <img
                      className=" w-full object-cover"
                      src={"http://localhost:3000/public/uploads/" + filename}
                      alt=""
                    />
                  </div>
                </div>
                ...
                <div className=" w-2/12  rounded h-full divide-x divide-gray-700  border-gray-600 border-opacity-50 p-[3px] flex items-center justify-between">
                  ...
                  >
                    <div
                      onClick={(e) => handleRemoveDraftImage(filename)}
                      accessKey="r"
                      tabIndex={index}
                      className="group  transform transition-transform duration-500 w-1/2 cursor-pointer h-full  flex justify-center items-center"
                    >
          ...
          {getDraftImage?.length > 0 && (
            <p className="text-gray-400 float-end text-[14px]">
              product-selected
            </p>
          )}
        </AnimatePresence>
        <AnimatePresence transition={{ duration: 0.5 }}></AnimatePresence>
        {previewImage && (
          <Image
            wrapperStyle={{
              display: "none",
